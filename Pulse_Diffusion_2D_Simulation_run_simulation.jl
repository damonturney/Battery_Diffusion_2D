...
struct simulation_results_struct
   ...
   voltage_timeseries              ::Array{Float64,1}
...
function run_simulation(ss, iterations, saved_iteration_spacing, voltage, dt)   # iterations=0:Int64(1E2)  , saved_iteration_spacing=1E0,  current_density=? ,  dt=5e-4
   println(" ");println(" ");println(" ");println(" ")
   println("Creating CyclicV_operator.")

   start_time=Dates.format(Dates.now(),"yyyymmddHHMMSS")   #start_time
   data_dictionary_name = start_time * "_dictionary_results.jld2" #jld2 is hdf5 format
   ...
   sim_data=simulation_results_struct(
      ...
      ,voltage*ones(length(iterations_saved))                                     #voltage_timeseries
      ...
      ,zeros(ss.num_x_mps + ss.spike_num_y_mps + 1)                               #Charge_Passed       WE COUNT THE CORNER MESHPOINT TWICE BECAUSE THEY HAVE INTERFACE POINTING IN THE DY DIRECTION AND THE DX DIRECTION
   ...
   overvoltage = sim_data.voltage_timeseries[1] .- voltage_eq_along_surface
   ...
   record_to_output_arrays(ss, sim_data, 1, 1, current_density, Charge_Passed, overvoltage, conc_A_along_surface)

   print("simulated duration is ",sim_data.dt[1]*sim_data.iterations[end], " seconds\n") 
   println("data saved to produced_data/"*data_dictionary_name) 
   ...
   for main_loop_iteration in 2:length(sim_data.iterations)
      ...
      for k in findall(sim_data.iterations_saved.==main_loop_iteration)
         ...
         record_to_output_arrays(ss, sim_data, k, main_loop_iteration, current_density, Charge_Passed, overvoltage, conc_A_along_surface)
      end
   ...
   save("produced_data/"*data_dictionary_name, Dict("system_state"=>ss,"sim_data"=>sim_data))
   ...
   println("sim_data = get(load(\"produced_data/"*data_dictionary_name*"\"), \"sim_data\",0);")
   println("ss =       get(load(\"produced_data/"*data_dictionary_name*"\"), \"system_state\",0);")
...
function record_to_output_arrays(ss, sim_data, k, main_loop_iteration, current_density, Charge_Passed, overvoltage, conc_A_along_surface)
      ...
      sim_data.Charge_Passed[:]           .= Charge_Passed[:]   # coulombs per m3
