...
struct pulse_voltage_then_current_simulation_data_structure
...
function pulse_voltage_then_current(ss, simulation_duration, dt_biggest, saved_dt_spacing, voltage_target, ontime1, superficial_cd_target, ontime2, electrode_voltage_limit)   # units are always mks  e.g. current = A/m2,  Flux = moles/m2/s,  concentration = moles/m3
   ...
   superficial_current_density_target                    = [superficial_cd_target]
   ...
   while time[1] <=  ss.accumulated_simulation_time[1] + simulation_duration
      ...
      if time[1] + 1E-13 >= save_data_time_thresholds[simdata_i]  #the + 1E-13 is because the computer can't store perfect numbers, e.g. 3E-5 can only be stored as 3.0000000000000004e-5
         ...
         #@printf("loop.%5.0i   superficial_current_density:%+0.7e   electrode_voltage:%+0.7e   target_cd:%+0.7e   superficial_cd_error:%+0.7e\n", main_loop_iteration, superficial_current_density , ss.electrode_voltage[1] , superficial_current_density_target[1] , superficial_current_density_error)
